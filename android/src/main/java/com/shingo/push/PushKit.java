...
import com.shingo.push.utils.MapUtils;
import com.shingo.push.xiaomi.XiaoMiPushMessageReceiver;
...
public class PushKit {
            ...
            FirebaseMessaging.getInstance().getToken()
                    .addOnCompleteListener(task -> {
                        ...
                        if (!TextUtils.isEmpty(token)) {
                            PushKit.instance.onToken(PushType.FCM, token);
                        }
                    });
        }
    ...
    private boolean isSupport(PushType pushType) {
        if (pushType == PushType.XiaoMi) {
            return !BuildConfig.XIAOMI_APP_ID.isEmpty() && !BuildConfig.XIAOMI_APP_KEY.isEmpty() && MiPushClient.shouldUseMIUIPush(context);
        }
        if (pushType == PushType.Huawei) {
            //判断是否支持华为推送(21年前部分荣耀手机也可能支持)
            return BuildConfig.HUAWEI_AGC_SERVICE_EXIST && HuaweiApiAvailability.getInstance().isHuaweiMobileNoticeAvailable(context) == ConnectionResult.SUCCESS;
        }
    ...
    public void handleIntent(Intent intent) {
        ...
        if (bundleExtras != null) {
            ...
            if (bundleExtras.containsKey("google.message_id") && isSupport(PushType.FCM)) {
            ...
            } else if (bundleExtras.containsKey(PushMessageHelper.KEY_MESSAGE) && isSupport(PushType.XiaoMi)) {
                ///小米推送
                ...
                Map<String, Object> message = MapUtils.toMap(BundleUtils.convertJSONObject(bundleExtras));
                PushKit.instance.onMessageClick(PushType.Oppo, message);
            }
        }
    }
